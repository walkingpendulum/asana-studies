...
class CreateWebhookThread(threading.Thread):
    def run(self):
        ...
        webhook = client.webhooks.create(resource=project,
                                         target="https://{0}.ngrok.io/receive-webhook?project={1}".format(
                                             ngrok_subdomain, project))
...
def teardown():
    ...
    while retries > 0:
        ...
        for hook in webhooks:
            try:
                client.webhooks.delete_by_id(hook["gid"])
                return "Deleted " + str(hook["gid"])
...
def receive_webhook():
    ...
    if "X-Hook-Secret" in request.headers:
        ...
        signature = hmac.new(hook_secret.encode('ascii', 'ignore'), msg=request.data, digestmod=hashlib.sha256).hexdigest()
